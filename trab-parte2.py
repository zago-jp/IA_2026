...
class WaypointsShortestPathProblem(SearchProblem):
    ...
    def add_waypoint(self, waypoint:int) -> None:
        try:
            ...
            dot = None
            op = None
            for i, dot in enumerate(waypoints):
                if i == waypoint:
                    op = waypoints[i-1]
                    dot = waypoints[i-1]
                    break

            print(f" Você Selecionou: {i} - {op['landmark'] if op is not None else 'Nenhum ponto selecionado'}")
            return self.waypoints.append(dot["landmark"])
